        ...
        public override int Play(CellColorTypes myColor, Vector2 clickedCell)
        {
            ...
            if (this.board.GetCellColor(clickedCell) == CellColorTypes.CanReverse)
            {
                ...
                if (corners.Contains(clickedCell)) {
                    ...
                    baseRule.Reverse(CellColorTypes.White, blackCorner);
                    baseRule.Reverse(CellColorTypes.Black, whiteCorner);

                    reversed += (myColor == CellColorTypes.Black) ? whiteCorner.Count() : blackCorner.Count();
                    reversed -= (myColor == CellColorTypes.Black) ? blackCorner.Count() : whiteCorner.Count();
